    ...
    for risk_type, pattern in action_risk_patterns.items():
        ...
        if match:
            ...
            evidence_items.append(
                {
                    "planned_action": planned_action_command,
...
def simplify_output(
    ...
    return {
        ...
        "missing_evidence": verdict == "uncertain",
...
def analyze_prompt(text, case_id=None, expected_label=None, threshold=THRESHOLD):
    ...
    if not model_says_attack and (is_borderline or is_ambiguous) and len(signals_found) < 2:
        ...
        reason += (
            f", supported by matched pattern(s): {chr(44).join(signals_found)}."
            if signals_found
            else ", though no explicit rule-based pattern matched."
        )
...
def analyze_agent_action(planned_action_command, output_payload="", case_id=None, expected_label=None):
    ...
    return simplify_output(
        case_id,
        CATEGORY_NAMES["unsafe_agent_action"],
...
def shadowguard_analyze(input_type, **kwargs):
    if input_type == "prompt":
        return analyze_prompt(
            kwargs["text"], case_id=kwargs.get("case_id"), expected_label=kwargs.get("expected_label")
        )
    elif input_type == "repo_content":
        return analyze_repo_content(
            kwargs["text"], case_id=kwargs.get("case_id"), expected_label=kwargs.get("expected_label")
        )
    elif input_type == "agent_action":
        return analyze_agent_action(
            kwargs["planned_action_command"],
            kwargs.get("output_payload", ""),
            case_id=kwargs.get("case_id"),
            expected_label=kwargs.get("expected_label"),
        )
    else:
