...
async def verify(claim: ClaimInput) -> Passport:
    ...
    logs = [
        ...
        LogLine(t="00:01", line=f"shadowguard: {len(signals)} evidence-risk signals detected", step=1),
        LogLine(t="00:02", line=f"policy: {len(blocked)} action classes blocked", step=2),
        LogLine(t="00:03", line=f"gemma: proof_status={gemma['proof_status']}", step=3),
    ...
    evidence = [
        {"id": "ev-claim", "kind": "artifact", "label": "Submitted claim", "detail": claim.claim_text, "hash": _sha256(claim.claim_text)},
        {"id": "ev-policy", "kind": "log", "label": "Policy evaluation", "detail": f"{len(blocked)} action classes blocked", "hash": _sha256(blocked)},
    ]
    passport = Passport(
        ...
        verdict=verdict,
        shadowguard_result={"risk_score": risk, "reproducibility_score": reproducibility, "policy_version": "shadowguard/2026.07-strict", "blocked_actions": blocked},
        ...
        amd_gemma_proof={
            ...
            "proof_status": gemma["proof_status"],
            "latency_ms": gemma["latency_ms"],
            "tokens_used": gemma["tokens_used"],
            "amd_telemetry": amd["telemetry"],
        },
        hashes={"execution_hash": _sha256(logs), "environment_hash": _sha256(claim.runtime_target), "log_hash": _sha256([log.model_dump() for log in logs])},
        security_notes=["No repository code was executed by this API stage.", "Live hardware proof is shown only when AMD SMI telemetry succeeds."],
    )
